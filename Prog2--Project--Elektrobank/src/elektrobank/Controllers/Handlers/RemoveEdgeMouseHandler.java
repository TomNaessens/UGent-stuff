...
public class RemoveEdgeMouseHandler extends MouseInputAdapter implements MouseHandler {

	private ModelBeheerder mBeheerder;
	private TekenModel tModel;

	public RemoveEdgeMouseHandler(ModelBeheerder mBeheerder) {
	...
	public void paint(Graphics g) {
		...
		for (EdgeContainer<Component, PointNode> edge : edges) {
			...
			if (tModel.getSelectedEdge() != null) {
				// Twee edges zijn gelijk als ze hetzelfde begin- en eindpunt hebben
				if (headNode.getLocation() == (tModel.getSelectedEdge().getHead()).getLocation()
					   && tailNode.getLocation() == (tModel.getSelectedEdge().getTail()).getLocation()) {
					g2.setColor(tModel.getDELETE_COLOR());
				} else {
