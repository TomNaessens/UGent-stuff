...
alter table question add constraint fk_question_server_14 foreign key (server_id) references file_server (id) on delete restrict on update restrict;
create index ix_question_server_14 on question (server_id);
alter table question_language add constraint fk_question_language_question_15 foreign key (question_id) references question (id) on delete restrict on update restrict;
create index ix_question_language_question_15 on question_language (question_id);
alter table set_language add constraint fk_set_language_set_16 foreign key (set_id) references set (id) on delete restrict on update restrict;
create index ix_set_language_set_16 on set_language (set_id);
alter table set_question add constraint fk_set_question_question_17 foreign key (question_id) references question (id) on delete restrict on update restrict;
create index ix_set_question_question_17 on set_question (question_id);
alter table set_question add constraint fk_set_question_set_18 foreign key (set_id) references set (id) on delete restrict on update restrict;
create index ix_set_question_set_18 on set_question (set_id);
