...
/*
 *  If this getUsername() returns a value, then the authenticator considers the user to be logged in as teacher, 
...
public class SecuredTeacher extends Security.Authenticator{
     ...
     * Only return value if user role is teacher
     */
    @Override
    public String getUsername(Context ctx) {
        if(ctx.session().get("role") == null)
            return null;
        if(ctx.session().get("role").equals("teacher") || ctx.session().get("role").equals("admin") || ctx.session().get("role").equals("organizer"))
    ...
    public Result onUnauthorized(Context ctx) {
        ...
        ctx.flash().put("error", LangMessages.get("notAuthorized.teacher", ctx.request().uri()));
