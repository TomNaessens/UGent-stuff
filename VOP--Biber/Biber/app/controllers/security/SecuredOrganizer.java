...
/*
 *  If this getUsername() returns a value, then the authenticator considers the user to be logged in as organizer, 
...
public class SecuredOrganizer extends Security.Authenticator{
    
    /*
     * (non-Javadoc)
     * @see play.mvc.Security.Authenticator#getUsername(play.mvc.Http.Context)
     * Only return value if user role is organizer
     */
    @Override
    public String getUsername(Context ctx) {
        if(ctx.session().get("role") == null)
            return null;
        if(ctx.session().get("role").equals("organizer") || ctx.session().get("role").equals("admin"))
    ...
    public Result onUnauthorized(Context ctx) {
        ...
        ctx.flash().put("error", LangMessages.get("notAuthorized.organizer", ctx.request().uri()));
