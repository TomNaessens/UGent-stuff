	...
	case "ENGLISH": 
      ...
      if (available.length > 0) {
        ...
        for (var i=0; i < languages.length; i++) {
          ...
          if (language.indexOf("_") != -1) {
			var lang="ENGLISH";
			if(language == "en_GB") lang = "ENGLISH";
			if(language == "nl_BE") lang = "DUTCH";
			if(language == "fr_FR") lang = "FRENCH";
			if(language == "de_DE") lang = "GERMAN";
			var url = $input.val();
			url += "?lang=" + lang;
			
            var languageArr = language.split("_")
            if (this.options.flags == true) {
              $options.append('<li><a tabindex="-1" href="' +url +'" data-option="' + language + '"><i class="icon-flag-' + languageArr[1] + '"></i>' + BFHLanguagesList[languageArr[0]].toProperCase() + '</a></li>')
            } else {
              $options.append('<li><a tabindex="-1" href="' +url +'" data-option="' + language + '">' + BFHLanguagesList[languageArr[0]].toProperCase() + ' (' + BFHCountriesList[languageArr[1]] + ')</a></li>')
            }
          } else {
            this.$element.append('<option value="' + language + '">' + BFHLanguagesList[language].toProperCase() + '</option>')
          }
