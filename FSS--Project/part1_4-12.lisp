...
;verificatie: als element wel in index, lengte pages list +1 als nog niet opgenomen
...
;verificatie: als element wel in index, lengte pages list +0 als reeds opgenomen
(defthm addEntry-adds-does-not-add-existing-pages-to-existing-words
  (implies (and (stringp a)
                (natp n)
                (indexp x)
                (member a (getWords x))
                (member n (getPages a x)))
           (eq (len (getPages a x)) (len (getPages a (addEntry a n x))))))
